...
def test_no_connection_case():
    assert get_last_value_for_metric('cpu', 'usage_system', 'telegraf', '127.0.0.1', '8080', 'telegraf', 'telegraf',
                                     logger) == 'No Connect'


def test_no_data_to_connect_case():
    with pytest.raises(AttributeError):
        get_last_value_for_metric('cpu', 'usage_system', 'telegraf', '127.0.0.1', None, 'telegraf', 'telegraf', None)


def test_db_does_not_exist_case():
    assert get_last_value_for_metric('cpu', 'usage_system', 'telegraf', '127.0.0.1', '8090', 'telegraf', 'telegraf',
                                     logger) is False
...
def test_measurements_in_db(create_db_with_measurments):
    assert get_last_value_for_metric('cpu', 'Float_value', 'telegraf', '127.0.0.1', '8090', 'telegraf', 'telegraf',
