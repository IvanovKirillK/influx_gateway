...
@pytest.fixture()
def create_db():
    client = InfluxDBClient('influxdb_remote', '8086', 'telegraf', 'telegraf', 'telegraf')
...
def test_db_does_not_exist_case(create_db):
    assert get_last_value_for_metric('cpu', 'usage_system', 'telegraf', 'influxdb_remote', '8086', 'telegraf1', 'telegraf',
                                     logger) is False


def test_no_measurements_in_db(create_db):
    assert get_last_value_for_metric('cpu', 'usage_system', 'telegraf', 'influxdb_remote', '8086', 'telegraf', 'telegraf',
                                     logger) is False
...
def test_measurements_in_db(create_db_with_measurments):
    assert get_last_value_for_metric('cpu', 'Float_value', 'telegraf', 'influxdb_remote', '8086', 'telegraf', 'telegraf',
